...
void DoorbellReceiver::apply_settings() {
    ...
    paired_valid_ = parse_paired_mac(paired_mac_);
...
bool DoorbellReceiver::parse_paired_mac(std::uint8_t out[6]) const {
    const std::string& s = preferences_.device().doorbell.paired_tx_mac;
    if (s.empty()) {
        return false;
    }
    unsigned b[6]{};
    // Accept AA:BB:CC:DD:EE:FF or AABBCCDDEEFF
    if (std::sscanf(s.c_str(), "%02x:%02x:%02x:%02x:%02x:%02x", &b[0], &b[1], &b[2], &b[3], &b[4],
                    &b[5]) == 6 ||
        std::sscanf(s.c_str(), "%02x%02x%02x%02x%02x%02x", &b[0], &b[1], &b[2], &b[3], &b[4],
                    &b[5]) == 6) {
        for (int i = 0; i < 6; ++i) {
